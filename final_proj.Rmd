...
#head(reduced.emissions)
ggpairs(reduced.emissions)
```

Transform using proportions, logs.
```{r}
basic.emissions.lm <- lm(emissions/total_population ~ log(household_income) + prop_pop_white + region + prop_dem, data=election.emissions)
...
```{r}
indicator.emissions.lm <- lm(emissions/total_population ~ log(household_income) + prop_pop_white + region + winning_party, data=election.emissions)
...
```{r}
resid_xpanel(indicator.emissions.lm)
resid_panel(indicator.emissions.lm, plots=c("resid","qq"))
```

```{r}
log.emissions.lm <- lm(log(emissions/total_population+1) ~ log(household_income) + prop_pop_white + region + winning_party, data=election.emissions)
summary(log.emissions.lm)
vif(log.emissions.lm)
anova(log.emissions.lm)
AIC(log.emissions.lm)
```

```{r}
resid_xpanel(log.emissions.lm, smoother=TRUE)
resid_panel(log.emissions.lm, plots=c("resid","qq"))
```

```{r}
sq.income.emissions.lm <- lm(log(emissions/total_population+1) ~ log(household_income) + I(log(household_income)^2) + prop_pop_white + region + winning_party, data=election.emissions)
...
```{r}
anova(log.emissions.lm, sq.income.emissions.lm)
```
...
```{r}
emissions.aug <- augment(sq.income.emissions.lm)
```
...
```{r}
r.sq.income.emissions.lm <- lm(log(emissions/total_population+1) ~ log(household_income) + I(log(household_income)^2) + prop_pop_white + region + winning_party, data=r.election.emissions)
summary(r.sq.income.emissions.lm)
vif(r.sq.income.emissions.lm)
anova(r.sq.income.emissions.lm)
AIC(r.sq.income.emissions.lm)
```

```{r}
resid_xpanel(r.sq.income.emissions.lm, smoother=TRUE)
resid_panel(r.sq.income.emissions.lm, plots=c("resid","qq"))
```

```{r}
r.emissions.lm <- lm(log(emissions+1) ~ log(household_income) + I(log(household_income)^2) + log(1-prop_pop_white) + region + winning_party, data=r.election.emissions)
summary(r.emissions.lm)
vif(r.emissions.lm)
anova(r.emissions.lm)
AIC(r.emissions.lm)
```

```{r}
resid_xpanel(r.emissions.lm, smoother=TRUE)
resid_panel(r.emissions.lm, plots=c("resid","qq"))
```

```{r}
quant.emissions.lm <- lm(log(emissions+1) ~ log(household_income) + I(log(household_income)^2) + log(1-prop_pop_white+0.01), data=r.election.emissions)
